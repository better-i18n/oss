...
/**
 * Better i18n MCP Content Server — HTTP transport
 *
 ...
 *
 * Usage:
 *   PORT=8808 better-i18n-mcp-content-http
 *
  ...
  if (pathname === "/mcp") {
    ...
    if (req.method === "POST" || req.method === "DELETE") {
      ...
      try {
        ...
        console.error("[better-i18n-content-http] Error handling MCP request:", err);
        if (!res.headersSent) {
...
httpServer.listen(port, host, () => {
  console.error(`[better-i18n-content-http] Mode: ${config.isLocalDev ? "LOCAL DEV" : "PRODUCTION"}`);
  console.error(`[better-i18n-content-http] API URL: ${config.apiUrl}`);
  console.error(`[better-i18n-content-http] MCP HTTP server listening on http://${host}:${port}/mcp`);
  console.error(`[better-i18n-content-http] Health check: http://${host}:${port}/health`);
});
