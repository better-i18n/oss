...
- 08a4039: `pull` no longer writes empty locale files behind a successful exit.
...
- 8621566: `sync --push` creates the missing keys in Better i18n after comparison (confirm prompt, `-y` to skip, `-p org/project` override). Fixed the crash when a project has never been published: the empty CDN manifest now produces a clear "publish once and retry" warning instead of `Cannot read properties of undefined` errors, and sync now prints the next step when it finds missing keys.

## 0.5.0

### Minor Changes

- 4f1323f: Add full translation management to CLI — login, keys, translations, publish, languages, and sync monitoring.
...
- 2b43a82: Auto-detect i18n hooks, fix config parsing, modernize terminal output
