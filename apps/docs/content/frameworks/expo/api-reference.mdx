...
When the CDN returns multi-namespace data (nested or flat format), the backend automatically registers **all** namespaces in i18next's resource store — not just the one that was requested. This means you don't need to manually configure or discover namespaces.

```
CDN response: { "common": {...}, "auth": {...}, "settings": {...} }

i18next requests "common" →
  ✓ Returns "common" data to i18next
  ✓ Auto-registers "auth" and "settings" in the resource store
```

Both nested (`{ "common": { "key": "value" } }`) and flat (`{ "common.key": "value" }`) formats are supported.
...
**Returns:** `Promise<BetterI18nResult>` — resolves when i18next is initialized
