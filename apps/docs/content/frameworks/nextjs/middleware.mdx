...
| `detection.explicitCookieName` | `string` | `"locale_explicit"` | Cookie that marks a *deliberate* user choice — written by `useSetLocale`, only **read** here. |
...
| `context.isExplicit` | `boolean` | `true` when the explicit-marker cookie is present (the user chose via the switcher) |
...
<Callout type="info">
  Set the same name on both sides: `detection.explicitCookieName` (middleware) and `explicitCookieName` (provider / `useSetLocale` config). Both default to `` `${cookieName}_explicit` ``.
</Callout>
