...
<AccordionItem title="getMessages(locale, options?)">
...
<Callout type="info">
Pass `namespaces` at [`createI18n` config time](/docs/frameworks/nextjs/configuration) to apply it to every call, or per-call here for page-specific overrides. See [Core — Selective Loading](/docs/core/selective-loading) for how batching and per-namespace caching work.
</Callout>
...
<AccordionItem title="getMessages(config, locale, options?)">
...
} from "@better-i18n/next/client";
```

<Accordion>
<AccordionItem title="BetterI18nProvider">
Enables instant client-side locale switching via `useSetLocale` — no page navigation or router
...
<AccordionItem title="useSetLocale(config?)">
