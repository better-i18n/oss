...
| `explicitCookieName` | `string` | `` `${cookieName}_explicit` `` | Cookie written by `useSetLocale` to mark a deliberate choice (match middleware's `detection.explicitCookieName`) |
...
| `namespaces` | `string[]` | — | Default namespaces to fetch on every call (namespaced_folders projects only). See [Selective Loading](/docs/core/selective-loading). |
...
<Tabs items={['Locale Prefix', 'Caching', 'Debug Mode']}>
  <Tab value="Locale Prefix">
    ...
    **`"never"`** — No locale in URLs. Locale is determined by cookie and `Accept-Language` header.
    ...
    <Callout type="warn">
      `"never"` mode requires specific file structure and middleware behavior. See [Middleware — Locale Prefix Modes](/frameworks/nextjs/middleware#locale-prefix-modes) for the full setup guide.
    </Callout>
  </Tab>
  <Tab value="Caching">
