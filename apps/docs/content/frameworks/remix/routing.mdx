...
export function LocaleSwitcher({ locale, locales }: LocaleSwitcherProps) {
  ...
  function handleSelect(newLocale: string) {
    ...
    const knownPrefixes = locales.filter((l) => l !== "en");
...
<Callout type="info">
The `locales` array comes from `i18n.getLocales()` — pass it through your root loader so the switcher always reflects available languages from the CDN manifest.
</Callout>
...
export default function App() {
  const { locale, locales, canonicalUrl } = useLoaderData<typeof loader>();
      ...
      <head>
        ...
        {locales.map((loc) => (
          <link
            ...
            rel="alternate"
            hrefLang={loc}
            href={canonicalUrl.replace(`/${locale}`, loc === "en" ? "" : `/${loc}`)}
          />
        ))}
        <Meta />
        <Links />
