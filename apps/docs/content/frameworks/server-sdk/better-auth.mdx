...
3. **Fetches** the translation from CDN (namespace: `auth`, key: the error code)
...
Client (Accept-Language: tr) → Better Auth → APIError
  → Provider: detectLocale("tr") → CDN: auth.INVALID_EMAIL_OR_PASSWORD
...
The provider also exports `DEFAULT_AUTH_KEYS` — a map of all 40+ Better Auth core error codes with their English defaults. Use it to seed your namespace:

```ts
import { DEFAULT_AUTH_KEYS } from "@better-i18n/server/providers/better-auth";

console.log(Object.keys(DEFAULT_AUTH_KEYS));
// ["INVALID_EMAIL_OR_PASSWORD", "USER_NOT_FOUND", "SESSION_EXPIRED", ...]
```

<Callout type="info">
  `DEFAULT_AUTH_KEYS` covers Better Auth **core** error codes. Plugin-specific codes (admin, two-factor, email-otp) can be added as additional keys in your namespace.
</Callout>
