...
When using [`@better-i18n/vite`](/frameworks/vite/setup), the provider reads everything from the injected `<script>` tag — no props needed:
...
See [Better Auth integration](/frameworks/server-sdk/better-auth#locale-detection) for the full pattern.
```
