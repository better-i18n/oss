...
3. Exchange code          →     /api/auth/mcp/token
   for tokens             ←     access_token + refresh_token + grant_id

4. Mint installation      →     POST /installations/:grantId/tokens
   token                  ←     bi_oat_... (1h, scoped)

5. Call resource APIs     →     GET /v1/keys (Bearer bi_oat_...)
                          ←     Translation data
```
...
- [Calling APIs](/oauth/calling-apis) — Use installation tokens with `/v1/*` endpoints
