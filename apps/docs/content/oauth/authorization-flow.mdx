...
<Callout type="warn">
**`client_secret` is required** for all confidential (server-side) clients. The server rejects the exchange without it. If you're using a PKCE-only public client, send `code_verifier` instead — see the [SPA flow](#spa-only-pkce-flow) below.
</Callout>
