...
Each grant is `(user × organization)`. To access a second org:

1. Start a new authorize flow
2. The consent screen lets the user pick a different org
3. You get a new `grant_id` + `refresh_token` for that org
4. Store one row per `(userId, organizationId)` in your database
...
interface BetterI18nConnection {
  ...
  organizationId: string;   // From /oauth-client/me
