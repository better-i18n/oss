...
<Callout type="info">
When `options.namespaces` is provided on a batch-enabled project (`manifest.batch === true`), the SDK collapses multiple namespace requests into a single HTTP call and caches each namespace individually for cross-page reuse. See [Selective Loading](/docs/core/selective-loading) for the full picture.
</Callout>
...
The `batch` and `namespaces` fields appear only for `namespaced_folders` projects served by CDN workers that support batching. See [Selective Loading](/docs/core/selective-loading) for how the SDK uses them.
