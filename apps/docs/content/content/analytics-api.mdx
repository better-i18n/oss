...
Returns the tracker context. Throws if used outside `<ContentProvider>`.
...
Inject the tracker. Throws if `provideContent()` wasn't called.
...
- [Analytics overview](/content/analytics) — concepts, transport, safety
- [Data Model](/content/analytics-data-model) — AE schema, what's stored where
