    ...
    <MarketingLayout showCTA={false} bgClassName="bg-white">
      ...
      <section >
        <div className="mx-auto max-w-3xl px-6 lg:px-10 text-center">
          <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full mb-6 ${ allOperational ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700" }`}>
            <div className={`size-2 rounded-full ${ allOperational ? "bg-emerald-500" : "bg-amber-500 animate-pulse" }`} />
        ...
        <div className="mx-auto max-w-3xl px-6 lg:px-10">
          <div className="rounded-xl border border-mist-200 overflow-hidden">
            {services.map((service, i) => (
              <div
                key={service.nameKey}
                className={`flex items-center justify-between p-4 ${ i !== services.length - 1 ? "border-b border-mist-100" : "" }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`size-2 rounded-full ${ service.status === "operational" ? "bg-emerald-500" : "bg-amber-500" }`} />
                ...
                <div className="flex items-center gap-4">
                  ...
                  <span className={`text-xs font-medium px-2 py-1 rounded ${ service.status === "operational" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700" }`}>
          ...
          <div className="flex gap-0.5">
            {Array.from({ length: 90 }).map((_, i) => (
              <div
                ...
                title={t("uptimeGraph.dayLabel", { day: String(90 - i) })}
              />
            ))}
          </div>
          ...
          {recentIncidents.length === 0 ? (
            ...
            <div className="space-y-4">
              {recentIncidents.map((incident) => (
                <div key={incident.title} className="p-4 rounded-lg border border-mist-200">
