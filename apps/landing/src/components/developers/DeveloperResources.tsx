...
const resourceKeys = [
  ...
  },
  {
    ...
    icon: <IconConsole className="size-3.5" />,
  },
  {
    key: "mcp",
    href: "https://docs.better-i18n.com/mcp",
    icon: <IconServer1 className="size-3.5" />,
  },
];
  ...
  return (
    <section>
      <div className="section">
        ...
        {/* Compact link list, the same rhythm as the compare hub and
            Alternatives: 22px mark, 15px title, 13px description, no card. It
            was four `p-6 rounded-xl border` cards with a 48px icon plate and a
            repeated "view docs" row per card — roughly twice the height for the
            same four links, and the repeated CTA said nothing the link did not
        ...
        <div className="grid grid-cols-1 gap-x-10 gap-y-6 sm:grid-cols-2 lg:grid-cols-4">
          {resourceKeys.map((resource) => (
            ...
            >
              <span className="mt-0.5 flex size-[22px] shrink-0 items-center justify-center rounded-sm border border-black/[0.04] bg-black/[0.03] text-mist-600">
                {resource.icon}
              </span>
              ...
              <SpriteIcon
                name="chevron-right"
                className="mt-1 size-3.5 shrink-0 text-mist-300 transition-[color,transform] group-hover:translate-x-0.5 group-hover:text-mist-600"
                aria-hidden="true"
              />
            </a>
          ))}
